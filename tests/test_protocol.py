...
class TestFrameEncode:
    def test_connect(self) -> None:
        assert encode_connect().encode() == bytes([0xC1, 0x00, 0x01, 0x01, 0x02])

    def test_disconnect(self) -> None:
        assert encode_disconnect().encode() == bytes([0xC1, 0x00, 0x01, 0x00, 0x01])

    def test_set_voltage_10v(self) -> None:
        raw = encode_set_voltage(10.0).encode()
        assert raw[0] == 0xB1  # START_WRITE
    ...
    def test_enable_output(self) -> None:
        ...
        assert encode_output_enable().encode() == bytes([0xB1, 0xDB, 0x01, 0x01, 0xDD])

    def test_disable_output(self) -> None:
        # Row 16335
        assert encode_output_disable().encode() == bytes([0xB1, 0xDB, 0x01, 0x00, 0xDC])

    def test_get_full_status_query(self) -> None:
        raw = encode_query(Cmd.GET_FULL_STATUS).encode()
        assert raw[1] == Cmd.GET_FULL_STATUS
        assert raw[2] == 1  # LEN = 1 (data = 0x00)
...
class TestKaitaiParse:
    def test_device_name(self) -> None:
        # a1 de 07 44 50 53 2d 31 35 30 8f  → "DPS-150"
        raw = bytes([0xA1, 0xDE, 0x07, 0x44, 0x50, 0x53, 0x2D, 0x31, 0x35, 0x30, 0x8F])
        p = FnirsiDps150(KaitaiStream(BytesIO(raw)))
        assert p.frame.payload.value == "DPS-150"

    def test_output_enable_echo(self) -> None:
        # a1 db 01 01 dd — device echoes back after enable
        raw = bytes([0xA1, 0xDB, 0x01, 0x01, 0xDD])
        p = FnirsiDps150(KaitaiStream(BytesIO(raw)))
        assert p.frame.payload.state == 1

    def test_output_disable_echo(self) -> None:
        # a1 db 01 00 dc
        raw = bytes([0xA1, 0xDB, 0x01, 0x00, 0xDC])
        p = FnirsiDps150(KaitaiStream(BytesIO(raw)))
        assert p.frame.payload.state == 0
    ...
    def test_push_output_values(self) -> None:
        ...
        p = FnirsiDps150(KaitaiStream(BytesIO(raw)))
        assert p.frame.payload.vout == pytest.approx(8.45, abs=0.01)
        assert p.frame.payload.iout == pytest.approx(0.0077, abs=0.001)
        assert p.frame.payload.pout == pytest.approx(
            p.frame.payload.vout * p.frame.payload.iout, rel=0.02
        )
...
class TestParseFrameErrors:
    def test_too_short(self) -> None:
        with pytest.raises(ProtocolError, match="too short"):
            parse_frame(b"\xa1\xde\x07")

    def test_truncated(self) -> None:
        with pytest.raises(ProtocolError, match="truncated"):
            parse_frame(bytes([0xB1, 0xC1, 0x04, 0x00, 0x00]))
    ...
    def test_correct_frame_round_trips(self) -> None:
        ...
        assert p.frame.cmd == Cmd.CONNECT_CTRL
    ...
    def test_frame_encode_no_dir_prefix(self) -> None:
        """Frame.encode() must NOT include the 0xf1 DIR byte — that's added by the transport."""
        raw = encode_connect().encode()
        assert raw[0] != 0xF1
        assert raw[0] == 0xC1  # START_CTRL
